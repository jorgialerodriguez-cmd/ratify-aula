...
// REGISTRO PWA (Para que se pueda instalar)
...
async function cambiarSeccion(seccion) {
    ...
    try {
        ...
        // Historial de Likes guardado en este móvil
        ...
        listaAudiosActual.forEach((audio, index) => {
            ...
            // ¿Es nuevo? (Subido hace menos de 7 días)
            ...
            item.innerHTML = `
                ...
                <div style="flex-grow: 1; overflow:hidden; padding-right:10px;">
                    <h4 style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; margin-bottom:5px; display:flex; align-items:center;">
                        ${audio.titulo} ${badgeHtml}
                    </h4>
                    <p>${audio.fecha}</p>
                </div>
            ...
            `;
            item.onclick = (e) => {
                // Prevenir que se reproduzca si tocamos el corazón o la papelera
                if(e.target.closest('.btn-like') || e.target.closest('.trash-btn')) return;
...
async function darLike(id, event) {
    ...
    const cajaLike = event.currentTarget;
    const icono = cajaLike.querySelector('i');
    const contador = cajaLike.querySelector('span');
    let misLikes = JSON.parse(localStorage.getItem('ratify_likes') || "{}");
    let delta = 1;

    if(misLikes[id]) {
        delete misLikes[id]; delta = -1; // Quitar Like
        icono.className = "far fa-heart"; icono.style.color = "var(--text-sub)";
    } else {
        misLikes[id] = true; delta = 1; // Dar Like
        icono.className = "fas fa-heart"; icono.style.color = "#ff4d4d";
    }
    
    localStorage.setItem('ratify_likes', JSON.stringify(misLikes));
    contador.innerText = (parseInt(contador.innerText) || 0) + delta;

    try { await window.updateDoc(window.doc(window.db, "episodios", id), { likes: window.increment(delta) }); } 
    catch(e) { console.log("Error guardando like en nube"); }
}
...
function togglePlay() {
    if(!audioReal.src) return;
    if (audioReal.paused) { audioReal.play(); actualizarBotonesPlay(true); } else { audioReal.pause(); actualizarBotonesPlay(false); }
}

function actualizarBotonesPlay(reproduciendo) {
    const iconos = document.querySelectorAll('.play-btn');
    iconos.forEach(icono => { if (reproduciendo) { icono.classList.remove('fa-play-circle'); icono.classList.add('fa-pause-circle'); } else { icono.classList.remove('fa-pause-circle'); icono.classList.add('fa-play-circle'); } });
}
function playSiguiente() { if (indiceAudioActual < listaAudiosActual.length - 1) reproducirAudio(indiceAudioActual + 1); }
function playAnterior() { if (indiceAudioActual > 0) reproducirAudio(indiceAudioActual - 1); }
audioReal.onended = playSiguiente;
...
async function ejecutarSubida() {
    ...
    try {
        ...
        await window.addDoc(window.collection(window.db, "episodios"), {
            titulo: titulo, seccion: seccion, url_audio: urlAudio, url_portada: urlPortada, fecha: fechaHoy,
            timestamp: Date.now(), likes: 0 // Preparado para la etiqueta de NUEVO y contador
        });

        alert("¡Episodio subido!"); document.getElementById('upload-titulo').value = '';
